#!/usr/bin/env python3
"""Re-check repaired nonsense files for remaining semantic issues."""
...
PROGRESS_LEDGER = REPO_ROOT / "tmp" / "lang_1_batches" / "nonsense_recheck_progress.txt"
FINDINGS_LOG = REPO_ROOT / "tmp" / "lang_1_batches" / "nonsense_recheck_findings.tsv"
TMP_DIR = REPO_ROOT / "tmp" / "lang_1_batches" / "nonsense_recheck"
...
def build_prompt(targets: list[dict]) -> str:
    ...
    joined = "\n\n".join(blocks)
    return f"""Review these repaired lang_1 files for remaining semantic nonsense.

Task:
- Check whether each file now makes basic sense for the target concept.
- Be conservative.
- Accept awkward but usable lines caused by tight vocabulary limits.
- Flag only clear remaining problems such as concept mismatch, category error, contradiction, or a localisation that clearly points to the wrong concept.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Re-check repaired lang_1 files for remaining semantic issues.")
    ...
    if not batch:
        ...
        print("findings log: tmp/lang_1_batches/nonsense_recheck_findings.tsv")
        return 0
