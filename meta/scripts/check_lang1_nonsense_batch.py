#!/usr/bin/env python3
"""Run one DeepSeek review batch for lang_1 sanity checking without editing files."""
...
SOURCE_LEDGER = OUT_DIR / "backfill_progress.txt"
PROGRESS_LEDGER = REPO_ROOT / "tmp" / "lang_1_batches" / "nonsense_check_progress.txt"
FINDINGS_LOG = REPO_ROOT / "tmp" / "lang_1_batches" / "nonsense_check_findings.tsv"
TMP_DIR = REPO_ROOT / "tmp" / "lang_1_batches" / "nonsense_check"
...
def load_targets() -> list[str]:
    return [line.strip() for line in SOURCE_LEDGER.read_text(encoding="utf-8").splitlines() if line.strip()]
...
def build_prompt(targets: list[str]) -> str:
    blocks: list[str] = []
    for word in targets:
        path = OUT_DIR / f"{slugify(word)}.md"
        try:
            content = path.read_text(encoding="utf-8").rstrip()
        except Exception as exc:
            content = f"<<READ ERROR: {exc}>>"
        blocks.append(f"FILE: training_data/lang/lang_1/{path.name}\nWORD: {word}\nCONTENT:\n{content}")
    joined = "\n\n".join(blocks)
    return f"""Review these lang_1 files for semantic nonsense or serious mismatch.

Task:
- Check whether the four lines in each file make basic sense for the target concept.
- Be conservative.
- Accept awkward but understandable sentences caused by tight vocabulary limits.
- Flag only clear problems such as category error, self-contradiction, obvious concept mismatch, malformed content, or a localisation that clearly points at the wrong concept.
- Do not suggest fixes.
- Do not rewrite anything.
- Return JSON only.

For each file:
- status = "ok" if the entry is basically usable, even if slightly awkward.
- status = "problem" only if it is meaningfully weird enough to review later.
- problem must be short and specific when status is "problem".
- problem must be an empty string when status is "ok".
...
- batch_problem must be an empty string unless the batch itself could not be evaluated.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run one DeepSeek batch for lang_1 nonsense checking.")
    ...
    if not targets:
        ...
        print("findings log: tmp/lang_1_batches/nonsense_check_findings.tsv")
        return 0
    ...
    for idx, max_tokens in enumerate(attempts, start=1):
        try:
            ...
            results = verify_batch(targets, parse_response(raw_text))
            findings = append_findings(results)
            append_ledger(targets)
            print(f"number of files checked: {len(targets)}")
