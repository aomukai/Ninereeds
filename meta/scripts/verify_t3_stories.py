"""
Verify tier-3 triplet stories against the allowlist.
Tier 3 rules: 3 paragraphs (9-14 sentences total), "because" at least twice,
  dialogue present, approved character present, allowlist compliance.
Writes summary to tmp/triplet_t3_violations.txt
"""
...
STORY_DIR = f"{REPO}/tmp/triplet_t3_output"
VIOLATIONS_OUT = f"{REPO}/tmp/triplet_t3_violations.txt"
...
def check_file(path):
    ...
    if ninereeds_block:
        ...
        n_sentences = count_sentences(story_text)
        if n_sentences < 7:
            issues.append(f"Too short: {n_sentences} sentences (need 7-16)")
        elif n_sentences > 16:
            issues.append(f"Too long: {n_sentences} sentences (need 7-16)")

        because_count = story_text.lower().count("because")
        if because_count < 1:
            issues.append('Missing "because" (causal reasoning marker, need at least 1)')
        ...
        if not found_chars:
            issues.append(f"No approved character name found")
