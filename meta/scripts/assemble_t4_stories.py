"""
Assemble verified tier-4 triplet stories into category files.
Reads from tmp/triplet_t4_output/*.md
Writes to training_data/triplet_stories/tier_4/<category>.md  (APPENDS to existing content)
"""
...
TIER_DIR = f"{REPO}/training_data/triplet_stories/tier_4"
VIOLATIONS = f"{REPO}/tmp/triplet_t4_violations.txt"
...
def main():
    ...
    for category in sorted(by_category.keys()):
        ...
        if os.path.exists(target):
            ...
            header = f"# {category.replace('_', ' ').title()} — Tier 4 Stories\n\n"
