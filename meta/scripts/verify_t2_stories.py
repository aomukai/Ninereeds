"""
Verify tier-2 triplet stories against the allowlist.
Usage:
  python3 verify_t2_stories.py              # check all files in tmp/triplet_t2_output/
  python3 verify_t2_stories.py path/to/file.md ...

Reports per-file: PASS / FAIL with flagged issues.
Writes summary to tmp/triplet_t2_violations.txt
"""
...
VIOLATIONS_OUT = f"{REPO}/tmp/triplet_t2_violations.txt"
...
def check_file(path):
    ...
    if ninereeds_block:
        ...
        if len(sentences) < 8:
            issues.append(f"Too short: {len(sentences)} sentences (need 8-16)")
        elif len(sentences) > 16:
            issues.append(f"Too long: {len(sentences)} sentences (need 8-16)")

        # Must have at least one dialogue line (quotation marks)
        if '"' not in story_text:
            issues.append("Missing dialogue (no quotation marks found)")

        # Must contain at least one approved character name
        found_chars = [c for c in CHARACTERS if re.search(r'\b' + c + r'\b', story_text.lower())]
        if not found_chars:
            issues.append(f"No approved character name found (expected one of: {sorted(CHARACTERS)})")
