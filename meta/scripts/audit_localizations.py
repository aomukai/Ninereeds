#!/usr/bin/env python3
"""
audit_localizations.py — Flag meaning-breaking literal translations in JP/ZH phase files.

For each localized file, sends the EN source + target sibling to an LLM and asks
it to identify expressions that a native speaker would find unnatural or wrong.

Output: JSONL audit log (one record per flagged file) at tmp/audit_<lang>.jsonl

Usage:
  python3 meta/scripts/audit_localizations.py run --lang JP [--phase 1] [--workers 2]
  python3 meta/scripts/audit_localizations.py run --lang ZH --phase all --workers 2
  python3 meta/scripts/audit_localizations.py report --lang JP
  python3 meta/scripts/audit_localizations.py report  # all langs

  # Use local LM Studio (Gemma 4 26B):
  python3 meta/scripts/audit_localizations.py run --lang JP --local
  python3 meta/scripts/audit_localizations.py run --lang JP --local --workers 2

  # Override endpoint/model manually:
  python3 meta/scripts/audit_localizations.py run --lang JP --endpoint http://192.168.3.5:1234/v1 --model gemma-4-26b-a4b-it
"""
...
def cmd_run(
    ...
    # Use relative path from ROOT for dedup key
    pending = [
        (src, loc) for src, loc in pairs
        if str(loc.relative_to(ROOT)) not in done
    ]
