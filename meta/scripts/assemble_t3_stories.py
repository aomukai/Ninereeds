"""
Assemble verified tier-3 triplet stories into category files.
Reads from tmp/triplet_t3_output/*.md
Writes to training_data/triplet_stories/tier_3/<category>.md  (APPENDS to existing content)
"""
...
TIER_DIR = f"{REPO}/training_data/triplet_stories/tier_3"
VIOLATIONS = f"{REPO}/tmp/triplet_t3_violations.txt"
...
def main():
    ...
    for fname in files:
        ...
        if not block:
            ...
            continue
        by_category.setdefault(category, []).append((fname, anchor, block))
    ...
    for category in sorted(by_category.keys()):
        ...
        if os.path.exists(target):
            ...
            header = f"# {category.replace('_', ' ').title()} — Tier 3 Stories\n\n"
