...
- checkpoint: `core/c17_contrast_review_repair_e1.pt`
...
```text
A dog is an animal. A dog is a mammal. A dog is often a pet. A dog is not a pers
```
...
```text
A tree is a plant. A tree has a trunk, branches, and leaves. A tree is not an an
```
...
```text
An airplane uses wings and engines to move through air. Engines push the airplan
```
