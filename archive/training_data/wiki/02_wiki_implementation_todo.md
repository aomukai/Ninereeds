...
42. [x] Run the post-Wiki-Level-2 quality pass on the dedicated Wiki Level 2 batch, one file at a time
   ...
   - Quality pass completed for `community_places_and_services_entries.md` (2026-04-25): PASS. Entry split verified (5/11). Advanced: library, hospital, grocery store, fire station, restaurant. Stayed L1: community place, service, police, police station, post office, museum, bakery, bus stop, bank, pharmacy, clinic. Article earns tokens through 5 well-scoped sections (25 Q&A pairs total): library (borrowing, quiet rules, lost books, librarian role), hospital (visit procedures, helpers, child fears, clinic comparison), grocery store (trip flow, child helping, separation safety), fire station (firefighter duties, emergency response, child safety, visiting), restaurant (ordering, manners, food dislikes, bakery comparison). Prose is child-facing with concrete scenarios and actionable safety guidance. Good coordination with health_and_wellness Level 2 (complementary—hospital visit experience here vs symptoms/medical care there). No drift from Phase 6 bridge growth. Stable before Level 3.
   - Quality pass completed for `technology_and_digital_media_entries.md` (2026-04-25): PASS. Entry split verified (5/9). Advanced: phone, tablet, computer, video, app. Stayed L1: technology, screen, message, swipe, tap, TV, keyboard, photo, username. Article earns tokens through 5 well-scoped sections (25 Q&A pairs total): The phone (rules, inappropriate times, permission, unknown callers, accidents), The tablet (phone comparison, good uses, time limits, ending screen time, download permission), The computer (uses, posture, strange behavior handling, password safety, device care), Videos (content concerns, real vs fiction, screen-time balance, emotional responses, stopping when asked), Apps (new app permission, grown-up permission features, in-app purchases, personal info requests, troubleshooting). Prose is child-facing with concrete scenarios and actionable safety guidance. Good coordination with online_safety_and_privacy Level 1 (complementary—technology covers device-specific scenarios, online_safety covers broader digital safety concepts). No drift from Phase 6 bridge growth. Stable before Level 3.
   - Quality pass completed for `health_and_wellness_entries.md` (2026-04-25): PASS. Entry split verified (12/5). Advanced: fever, cough, sore throat, headache, stomachache, cut, bruise, medicine, germ, sneeze, runny nose, checkup. Stayed L1: health, wellness, bandage, rash, allergy. Article earns tokens through 12 well-scoped subsections in 3 categories (56 Q&A pairs total): Common symptoms (Fever, Cough, Sore throat, Headache, Stomachache), Visible injuries (Cut, Bruise), Care and prevention (Medicine, Germ, Sneeze, Runny nose, Checkup). Prose is child-facing with actionable care guidance and when-to-tell-adults scripts. Good coordination with community_places_and_services Level 2 (complementary—hospital visit experience there vs symptoms/medical care here). No drift from Phase 6 bridge growth. Stable before Level 3.
   - Quality pass completed for `storytelling_and_narrative_structure_entries.md` (2026-04-25): PASS. Entry split verified (5/11). Advanced: story, plot, narrator, suddenly, meanwhile. Stayed L1: beginning, middle, end, first, next, then, before, after, finally, at the end, once upon a time. Article earns tokens through 5 well-scoped sections (20 Q&A pairs total): Story (story types, book vs story, good story qualities, oral storytelling), Plot (definition, necessity, problems, endings), Narrator (definition, "I" vs external, perspective differences, function), Suddenly (purpose, what follows, excitement creation, overuse caution), Meanwhile (purpose, parallel threads, difference from "then", multiple meanwhiles). Prose is child-facing and concrete. Good coordination with logic_entries (beginning/middle/end), imagination_and_pretend_play_entries (pretend), story_roles_and_plot_elements_entries (character), and school_life_and_learning_entries (book). No drift from Phase 6 bridge growth—bridge file 06 teaches sequence vocabulary that supports but doesn't overlap narrative structure. Stable before Level 3.
   - Quality pass completed for `perspective_taking_and_theory_of_mind_entries.md` (2026-04-25): PASS. Entry split verified (6/2). Advanced: perspective, believe, misunderstand, I thought, he didn't know that, put yourself in someone else's place. Stayed L1: they felt, she wanted (correctly deferred to emotions and wants/needs domains). Article earns tokens through 6 well-scoped concept sections with concrete examples and key takeaway guidance, plus 3 common theory-of-mind scenarios (hidden object, changed label, secret message). Prose is child-facing and concrete. Good coordination with logic_entries (fact, true), emotions_entries (they felt deferred), wants_needs_and_preferences_entries (she wanted deferred). No drift from Phase 6 bridge growth—bridge file 03 teaches thought/think/know/understand and file 04 teaches true/real/fact/reason, which support but don't overlap perspective-taking concepts. Stable before Level 3.
   - Quality pass completed for `evidence_and_justification_entries.md` (2026-04-25): PASS. Entry split verified (4/5). Advanced: justification, reason why, example, that proves. Stayed L1: because I saw it, I know this because, for instance, I can show you, back it up. Article earns tokens through 6 well-scoped sections: Justification (5 subsections covering definition, 4 sources, weak patterns, classroom use), Reason Why (5 subsections covering definition, 3 kinds, 4 sources, reason vs example), Example (5 subsections covering definition, purpose, good vs weak, quantity, examples vs proof), That Proves (5 subsections covering definition, strong vs weak usage, prove vs support), How concepts connect (summary), Common scenarios (3 situations). Prose is child-facing with concrete examples. Good coordination with logic_entries (proof, fact, reason—complementary), Phase 6 bridge file 04 (true/real/fact/reason taught at curriculum level). No drift from bridge/story growth. Stable before Level 3. **Post-Wiki-Level-2 quality pass COMPLETE for all 12 files.**
...
43. [x] Run the post-Wiki-Level-2 quality pass on Story Tier 2, one file at a time
   ...
   - Quality pass completed for `school_and_learning.md` (2026-04-25): PASS. All 20 stories verified against Tier 2 spec. 12 sentences per story, named characters (19 unique in character_registry.md), anchor + 2 supports per triplet, one mild obstacle per story, quoted dialogue with explicit speaker tags (sparing, 1 per story), no elliptical dialogue, endings inside scene. Noun → name → pronoun alternation clear throughout. Character reuse consistent (Sam in pencil/recess; Ms. Hall in teacher/lesson; Ava in eraser/lesson). No drift from Phase 6 bridge or Wiki Level 2. Stable before later tiers. Next file: `play_and_games.md`.
...
44. [x] Only after Tasks 40–43 are complete, open the Wiki Level 3 planning/review gate
   ...
   - Gate opened 2026-04-25: Created `wiki_level3_planning_gate.md` documenting prerequisite completion (Tasks 40-43), Level 3 expansion criteria, and candidate assessment. Only `emotions_entries.md` has Level 3 ceiling; all others capped at Level 2. Story Layer 2 must complete across all domains before Level 3 article writing begins.
   - Clarification: this gate is for planning only and does **not** mean the dedicated `wiki_level2_queue.md` dependency-pass bookkeeping is fully closed out. Use the Level 2 queue / manifest / expansion indexes as the source of truth for Level 2 production-state questions.

### E. Current active queue — post-Level-2 progression

45. [ ] Finish the Tier 1 story audit queue one domain file at a time using `training_data/triplet_stories/review_queue.md`
   Notes:
   - Work the first unchecked domain file only.
...
49. [ ] Create Wiki Level 3 articles one file container at a time
   Notes:
   - Use `training_data/wiki/wiki_level3_planning_gate.md`.
   - If `training_data/wiki/wiki_level3_queue.md` is missing, create the minimal canonical queue first.
   - Only advance files or entries that genuinely earn Level 3.

50. [ ] Run the Wiki Level 3 quality/review pass one file container at a time
   Notes:
...
52. [ ] Run the Tier 3 quality-pass queue one domain file at a time
   Notes:
   - If `training_data/triplet_stories/tier_3/post_review_queue.md` is missing, create the minimal canonical review queue first.

53. [ ] Create Wiki Level 4 articles one file container at a time
   Notes:
   - If `training_data/wiki/wiki_level4_queue.md` is missing, create the minimal canonical queue first.
   - Only advance files or entries that genuinely earn Level 4.

54. [ ] Run the Wiki Level 4 quality/review pass one file container at a time
   Notes:
...
56. [ ] Run the Tier 4 quality-pass queue one domain file at a time
   Notes:
   - If `training_data/triplet_stories/tier_4/post_review_queue.md` is missing, create the minimal canonical review queue first.

57. [ ] Build and maintain a canonical list of concepts introduced in wiki and story layers
   Notes:
   - Use `training_data/wiki/wiki_and_story_introduced_concepts.md` as the canonical artifact; create it if missing.

58. [ ] Implement the newly introduced wiki/story concepts into the Phase 1-5 curriculum files, one bounded batch at a time
   Notes:
   - Use the canonical introduced-concepts list from Task 57.
   - Keep curriculum edits reproducible and dependency-shaped.

59. [ ] Update `training_data/phase 1 to 5/rewritten/dependency_graph.json` after the curriculum backfill work lands
