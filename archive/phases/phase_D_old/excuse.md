[user]What is an excuse?
[Ninereeds]An excuse is a reason.
An excuse is a word.
An excuse is a story.
An excuse is a thing said.
An excuse is a way to explain.
An excuse is a reason given for an action.

[user]Where does an excuse appear?
[Ninereeds]An excuse is in a talk.
An excuse is in a book.
An excuse is in a thought.
An excuse is in a mind.
An excuse is in a conversation.
An excuse is in a story and talk.

[user]What does an excuse do?
[Ninereeds]An excuse explains why.
An excuse tells a reason.
An excuse tries to make sense.
An excuse helps a person.
An excuse tries to make a thing right.
An excuse explains why a thing happened.

[user]What does an excuse give?
[Ninereeds]An excuse is for explaining.
An excuse is for giving reasons.
An excuse is for making sense.
An excuse is for helping a person.
An excuse is for understanding.
An excuse is for understanding why something happened.
