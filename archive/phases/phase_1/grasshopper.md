[user]What does a grasshopper look like?
[Ninereeds]A grasshopper has six legs.
A grasshopper has two antennae.
A grasshopper has long back legs.
A grasshopper is green or brown.
A grasshopper has wings on the back.
A grasshopper is an insect with six legs and long back legs.

[user]Where can you find a grasshopper?
[Ninereeds]A grasshopper is in the grass.
A grasshopper is in a field.
A grasshopper is on a leaf.
A grasshopper is on a plant.
A grasshopper is in a garden.
A grasshopper is in the grass and in a field and on a leaf.

[user]What does a grasshopper do?
[Ninereeds]A grasshopper jumps in the air.
A grasshopper hops on the ground.
A grasshopper lands on a blade of grass.
A grasshopper rubs the legs together.
A grasshopper makes a chirp sound.
A grasshopper jumps in the air and hops on the ground.

[user]What does a grasshopper give?
[Ninereeds]A grasshopper is for a bird.
A grasshopper is for a lizard.
A grasshopper is for a frog.
A grasshopper is for a spider.
A grasshopper is for a food chain.
A grasshopper is for a bird and a lizard and a frog.
