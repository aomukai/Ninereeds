[user]What does a shadow look like?
[Ninereeds]A shadow is dark.
A shadow is flat.
A shadow follows a shape.
A shadow is on the ground.
A shadow is behind a thing.
A shadow is a dark flat shape on the ground.

[user]Where can you find a shadow?
[Ninereeds]A shadow is on the ground.
A shadow is on a wall.
A shadow is behind a person.
A shadow is next to a thing.
A shadow is opposite the sun.
A shadow is on the ground opposite the sun.

[user]What does a shadow do?
[Ninereeds]A shadow moves with the sun.
A shadow changes shape.
A shadow gets long and short.
A shadow points away from light.
A shadow follows a person.
A shadow moves and changes shape with the sun.

[user]What does a shadow give?
[Ninereeds]A shadow is for play.
A shadow is for making shapes.
A shadow is for telling time.
A shadow is for hiding.
A shadow is for watching.
A shadow is for play and making shapes.
