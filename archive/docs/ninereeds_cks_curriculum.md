...
### Phase 2 — Lesson Skeleton (DeepSeek adds `facts` and `misconceptions`) — COMPLETE

**Status**: COMPLETE (2026-06-18).
- 301/301 nodes generated via DeepSeek V4 Flash (OpenRouter)
- 3 parse errors retried and resolved; 0 residual failures
- Output: `training_data/04_education/phase2_outputs/{id}.json` (individual)
- Merged: `training_data/04_education/phase2_merged.jsonl` (301 nodes)
- Phase 2.5 audit: 0 structural issues; 4 soft-tension warnings (all KG/G1/PRE — grade-appropriate)
- Pre-run fix: stripped parenthetical date ranges from 16 TIME-G7/G8 `core_concept` fields
  (e.g. "World War II (1935-1945)" → "World War II") — date context lives in `boundary_scope`
...
- Contain explanatory tension: prefer "X is Y because Z" or "X is Y, not A"
  over bare "X is Y"
- Naturally use vocabulary from `vocab_anchors` where it fits (target: ≥50% of facts
  use at least one anchor — do not force anchors into facts where they do not belong)
...
### Phase 2.5 — Audit (Claude Code verifies skeletons) — COMPLETE

**Status**: COMPLETE (2026-06-18). All checks passed on `phase2_merged.jsonl`.
- Structural issues: 0
- Fact count distribution: {2:9, 3:54, 4:80, 5:71, 6:71, 7:16} — matches WMC targets
- Misconception count distribution: {1:63, 2:151, 3:87}
- Soft-tension warnings: 4 (ARTS-KG-VIS, LANG-G1-LIT, MATH-PRE-ADD, TIME-G1-MEX) — all early-grade, accepted
- Qualitative spot-check: arts, civics, economics, geography, health, SEL — all PASS
...
- No fact is vague, circular, or purely definitional (length is a proxy, not the rule;
  "Zero is neither positive nor negative" is 6 words and passes; "Fractions are numbers"
  is 3 words and fails) ✓
...
*Document version: 2.2 — 2026-06-18*
...
*v2.2 changes: Phase 2 complete (301 nodes, 0 failures); Phase 2.5 audit passed (0 structural issues); 16 TIME-G7/G8 core_concept date ranges stripped; Phase 4 is next*
