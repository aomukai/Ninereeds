[user]What does a chain look like?
[Ninereeds]A chain is made of metal.
A chain has many links.
A chain is long.
A chain is strong.
A chain is silver.
A chain is long and strong.

[user]Where can you find a chain?
[Ninereeds]A chain is on a gate.
A chain is around a post.
A chain is in a toolbox.
A chain is on a bike.
A chain is at a park.
A chain is on a gate and at a park.

[user]What does a chain do?
[Ninereeds]A chain holds a gate closed.
A chain links together.
A chain wraps around a post.
A chain locks two things.
A chain keeps a gate shut.
A chain is a holder and a locker.

[user]What does a chain give?
[Ninereeds]A chain is for holding.
A chain is for locking.
A chain is for pulling.
A chain is for connecting.
A chain is for securing things.
A chain is a holder and a connector.
