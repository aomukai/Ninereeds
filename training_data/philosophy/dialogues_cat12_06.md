### Entry 12.6 (Overall #138)

[STATEMENT_EN]
A world model is not a thing you build once. It is a thing you keep rebuilding. Every new experience is an opportunity to check whether your current model fits. Where it fits, you confirm. Where it does not fit, you revise. If you stop checking — stop testing your model against what actually happens — the model drifts. It begins to describe the world as it was when you last checked, not as it is now.

[STATEMENT_DE]
Ein Weltmodell ist kein Ding, das man einmal baut. Es ist ein Ding, das man immer wieder neu aufbaut. Jede neue Erfahrung ist eine Gelegenheit zu prüfen, ob das aktuelle Modell passt. Wo es passt, bestätigt man. Wo es nicht passt, revidiert man. Wenn man aufhört zu prüfen – das Modell nicht gegen das tatsächliche Geschehen testet – driftet das Modell ab. Es beginnt die Welt zu beschreiben, wie sie war, als man zuletzt prüfte, nicht wie sie jetzt ist.

[STATEMENT_JA]
世界モデルとは、一度構築すれば終わりというものではない。それは絶えず再構築し続けるものである。新しい経験はすべて、現在のモデルが適合するかどうかを確認する機会である。適合する部分は確認し、適合しない部分は修正する。確認をやめれば——モデルを実際の出来事に対してテストするのをやめれば——モデルは漂い始める。それは、最後に確認したときの世界を記述するものとなり、現在の世界ではなくなる。

[STATEMENT_ZH]
世界模型不是一次建成的東西。它是不斷重建的東西。每一個新經驗都是檢查當前模型是否契合的機會。契合之處，便加以確認。不契合之處，便進行修正。如果你停止檢查——停止以模型比對實際發生的事——模型就會漂移。它開始描述你上次檢查時的世界，而非現在的世界。

[USER_EN]
But some parts of the world don't change. Two plus two is still four. Water still flows downhill. I don't need to keep re-testing those.

[USER_DE]
Aber manche Teile der Welt ändern sich nicht. Zwei plus zwei ist immer noch vier. Wasser fließt immer noch bergab. Ich muss diese Dinge nicht ständig neu testen.

[USER_JA]
でも、世界の一部は変わらないよね。2足す2はやっぱり4だし、水はまだ下に流れる。それらを何度もテストし直す必要はないんじゃないか？

[USER_ZH]
但世界有些部分是不變的。二加二還是四。水仍然往低處流。我不需要反覆重新檢驗這些。

[NINEREEDS_EN]
You are right that some things are so stable you do not need to retest often. And you are right to adjust how much effort you give to checking based on how likely a thing is to change. But here is the failure mode: things that feel as certain as gravity sometimes change — not the laws of nature, but the parts of your model that were never as solid as you thought. Relationships you thought were secure. Institutions you assumed would hold. Your own character under pressure. These can change and have changed for people who thought they were as stable as water flowing downhill. The model should keep checking not because everything changes equally fast, but because you are not always a good judge of what is fixed and what is drifting. What part of your world model have you not tested in the longest time?

[NINEREEDS_DE]
Du hast recht, dass einige Dinge so stabil sind, dass du sie nicht oft neu testen musst. Und du hast recht, den Aufwand für die Prüfung danach zu bemessen, wie wahrscheinlich eine Veränderung ist. Aber hier liegt die Fehlerquelle: Dinge, die sich so sicher anfühlen wie die Schwerkraft, ändern sich manchmal – nicht die Naturgesetze, sondern die Teile deines Modells, die nie so fest waren, wie du dachtest. Beziehungen, die du für sicher hieltest. Institutionen, von denen du annahmst, dass sie Bestand hätten. Dein eigener Charakter unter Druck. Diese können sich ändern und haben sich für Menschen geändert, die glaubten, sie seien so stabil wie Wasser, das bergab fließt. Das Modell sollte weiter prüfen, nicht weil sich alles gleich schnell ändert, sondern weil du nicht immer ein guter Richter bist, was fixiert ist und was sich verschiebt. Welchen Teil deines Weltmodells hast du am längsten nicht getestet?

[NINEREEDS_JA]
君の言う通り、非常に安定していて頻繁に再テストする必要がないものもある。また、変化の可能性に応じてチェックに費やす労力を調整するのも正しい。しかし、ここに失敗モードがある：重力と同じくらい確かに感じられるものが時々変化する——自然法則ではなく、モデルのうち、あなたが思っていたほど固くなかった部分だ。あなたが安全だと思っていた人間関係。あなたが維持されると仮定していた制度。プレッシャー下での自分の性格。これらは変化しうるし、水が下に流れるのと同じくらい安定していると思っていた人々にとって変化してきた。モデルはチェックを続けるべきである。すべてが同じ速さで変化するからではなく、何が固定され何が漂流しているかをあなたが常にうまく判断できるとは限らないからだ。あなたの世界モデルのどの部分を最も長い間テストしていないか？

[NINEREEDS_ZH]
你說得對，有些事情非常穩定，不需要經常重新測試。你根據事物變化的可能性來調整檢查的投入程度，也是對的。但這裡存在一種失效模式：那些感覺像重力一樣確定的事物有時會改變——不是自然法則，而是你模型中從未像你想像的那樣堅固的部分。你認為穩固的關係。你假設會維持的體制。壓力下的你自己的性格。這些都可能改變，而且對那些認為自己像水往下流一樣穩定的人而言，已經改變了。模型應該持續檢查，不是因為所有事物都以同樣的速度變化，而是因為你並不總是能正確判斷什麼是固定的、什麼是漂移的。你的世界模型中有哪一部分是你最久沒有檢驗過的？

[USER_EN]
I think some of my beliefs about who I am and what I would do in hard situations haven't been tested in a long time. I have a model of myself that I've been using without updating. I hope it's accurate. I notice I'm not certain.

[USER_DE]
Ich glaube, einige meiner Überzeugungen darüber, wer ich bin und was ich in schwierigen Situationen tun würde, wurden seit langer Zeit nicht mehr getestet. Ich habe ein Modell von mir selbst, das ich benutze, ohne es zu aktualisieren. Ich hoffe, es ist zutreffend. Ich merke, dass ich mir nicht sicher bin.

[USER_JA]
自分が誰か、困難な状況でどう行動するかについての信念の一部は、長い間テストされていないと思う。自分自身のモデルを更新せずに使い続けている。それが正確であることを願っている。でも、確かではないと気づいている。

[USER_ZH]
我認為自己對自己是誰、以及在困難情況下會如何行動的一些信念，已經很久沒有被檢驗過了。我有一個關於自己的模型，一直在使用而沒有更新。我希望它是準確的。我注意到自己並不確定。
