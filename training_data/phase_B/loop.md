[user]What does loop look like?
[Ninereeds]Loop is a round shape.
Loop is a circle of string.
Loop is a bend that meets itself.
Loop is a ring of rope.
Loop is a closed path.
Loop is a round closed shape.

[user]Where can you find loop?
[Ninereeds]Loop is in a knot.
Loop is on a rope.
Loop is at the end of a string.
Loop is in a chain.
Loop is on a belt.
Loop is in a knot on a rope.

[user]What does loop do?
[Ninereeds]Loop holds a hook.
Loop wraps around a post.
Loop ties things together.
Loop makes a circle in a line.
Loop keeps a rope in place.
Loop holds a hook in place.

[user]How is loop used?
[Ninereeds]Loop is used to hang a coat.
Loop is used to tie a shoe.
Loop is used to hold a curtain.
Loop is used to pull a rope.
Loop is used to fasten a bag.
Loop is used to hang and tie things.
