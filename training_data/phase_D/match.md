[user]What does match mean?
[Ninereeds]Match is to be the same.
Match is to be alike.
Match is to be a pair.
Match is to fit together.
Match is to correspond.
Match is to be the same and alike.

[user]Where does match appear?
[Ninereeds]Match appears in colors.
Match appears in shapes.
Match appears in games.
Match appears in ideas.
Match appears in feelings.
Match is to find things that are the same.

[user]What does match do?
[Ninereeds]Match makes things complete.
Match makes things fit.
Match makes things harmonize.
Match makes things work well.
Match makes things balanced.
Match makes things complete and work well.

[user]What does match give?
[Ninereeds]Match is for finding pairs.
Match is for making sets.
Match is for connecting things.
Match is for solving puzzles.
Match is for creating order.
Match is for finding pairs and creating order.
