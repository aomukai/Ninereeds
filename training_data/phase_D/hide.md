[user]What does hide look like?
[Ninereeds]Hide is a quick duck from view.
Hide is a still cover of body.
Hide is a secret tuck from sight.
Hide is a quiet stay out of light.
Hide is a careful slip from notice.
Hide is a quick duck from view.

[user]Where does hide happen?
[Ninereeds]Hide happens behind a tree.
Hide happens under a bed.
Hide happens in a closet.
Hide happens behind a curtain.
Hide happens in a cave.
Hide happens behind a tree and under a bed.

[user]What does hide do?
[Ninereeds]Hide ducks behind a big tree.
Hide crawls under a soft bed.
Hide slips into a dark closet.
Hide covers face with two hands.
Hide stays still in a quiet spot.
Hide ducks behind a big tree.

[user]What does hide give?
[Ninereeds]Hide keeps a person out of sight.
Hide protects a child from danger.
Hide stores a toy in a secret spot.
Hide covers a gift until a party.
Hide shelters an animal from a predator.
Hide keeps a person out of sight.
