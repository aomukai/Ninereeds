[user]What does an edge look like?
[Ninereeds]An edge is a line.
An edge is a border.
An edge is a limit.
An edge is sharp.
An edge is thin.
An edge is a thin, sharp line.

[user]Where can you find an edge?
[Ninereeds]An edge is on a table.
An edge is on a paper.
An edge is on a cliff.
An edge is at the end.
An edge is a dividing line.
An edge is on a table at the end.

[user]What does an edge do?
[Ninereeds]An edge defines space.
An edge separates areas.
An edge can cut.
An edge can hold.
An edge can be a point of change.
An edge defines and separates areas.

[user]What does an edge give?
[Ninereeds]An edge is for defining.
An edge is for cutting.
An edge is for stability.
An edge is for protection.
An edge is for observation.
An edge is for defining and cutting.
