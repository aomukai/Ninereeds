[user]What does a file look like?
[Ninereeds]A file is a folder.
A file is paper.
A file is thin.
A file is brown.
A file is in a computer.
A file is paper and data.

[user]Where does a file appear?
[Ninereeds]A file is in a cabinet.
A file is on a computer.
A file is on a desk.
A file is in a drawer.
A file is on a shelf.
A file is in a cabinet and on a desk.

[user]What does a file do?
[Ninereeds]A file stores information.
A file organizes papers.
A file holds data.
A file keeps records.
A file archives documents.
A file is a store and an organizer.

[user]What does a file give?
[Ninereeds]A file is for keeping documents.
A file is for saving work.
A file is for backup.
A file is for sharing data.
A file is for archiving records.
A file is for keeping and for sharing.
