[user]What does an actor look like?
[Ninereeds]An actor is a person.
An actor is expressive.
An actor is talented.
An actor is a performer.
An actor is a storyteller.
An actor is an expressive and talented person.

[user]Where can you find an actor?
[Ninereeds]An actor is on a stage.
An actor is in a film.
An actor is on TV.
An actor is in a play.
An actor is in front of a camera.
An actor is on a stage and in a film.

[user]What does an actor do?
[Ninereeds]An actor plays a role.
An actor portrays a character.
An actor tells a story.
An actor entertains people.
An actor creates emotion.
An actor plays a role and entertains people.

[user]What does an actor give?
[Ninereeds]An actor is for entertainment.
An actor is for storytelling.
An actor is for artistic expression.
An actor is for bringing characters to life.
An actor is for inspiring.
An actor is for entertainment and storytelling.
