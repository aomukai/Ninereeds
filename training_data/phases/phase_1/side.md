[user]What does a side look like?
[Ninereeds]A side is flat.
A side is a face.
A side has edges.
A side is part of a shape.
A side is left or right.
A side is a flat face of a shape.

[user]Where can you find a side?
[Ninereeds]A side is on a box.
A side is on a square.
A side is on a house.
A side is along an edge.
A side is a surface area.
A side is on a box and on a square.

[user]What does a side do?
[Ninereeds]A side borders a shape.
A side meets another side.
A side forms a corner.
A side holds the shape.
A side is a boundary.
A side borders a shape and meets another side.

[user]What does a side give?
[Ninereeds]A side makes a shape.
A side is a surface.
A side is a face for labels.
A side is a wall.
A side is an edge line.
A side makes a shape and is a surface.
