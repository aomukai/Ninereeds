[user]What does a machine look like?
[Ninereeds]A machine is big.
A machine has parts.
A machine has wires.
A machine has a motor.
A machine is made from metal and plastic and glass.
A machine is big and has a motor.

[user]Where can you find a machine?
[Ninereeds]A machine is in a factory.
A machine is in a shop.
A machine is on a floor.
A machine is in a room.
A machine is on a table and in a garage.
A machine is in a factory and on a floor.

[user]What does a machine do?
[Ninereeds]A machine cuts parts.
A machine moves parts.
A machine makes noise.
A machine spins a wheel.
A machine shapes metal and wood into parts.
A machine cuts parts and spins a wheel.

[user]What does a machine give?
[Ninereeds]A machine is for work.
A machine is for cutting.
A machine is for moving.
A machine is for making things.
A machine helps a person build and make parts.
A machine is for cutting and moving things.
