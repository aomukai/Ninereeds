[user]What does an oak look like?
[Ninereeds]An oak is a tree.
An oak is tall.
An oak has a thick trunk.
An oak has many branches.
An oak has green leaves.
An oak is a tall tree with green leaves and many branches.

[user]Where can you find an oak?
[Ninereeds]An oak is in a forest.
An oak is in a park.
An oak is in a yard.
An oak is on a hill.
An oak grows from acorns in the ground.
An oak is a tree that grows in forests and parks.

[user]What does an oak do?
[Ninereeds]An oak grows tall and wide.
An oak drops acorns in autumn.
An oak gives shade in summer.
An oak holds birds in branches.
An oak makes oxygen for the air.
An oak is a tree that gives shade and holds birds.

[user]What does an oak give?
[Ninereeds]An oak is for shade.
An oak is for wood.
An oak is for climbing.
An oak is for homes for birds.
An oak is for making the world green.
An oak is a tree for shade, wood, and climbing.
