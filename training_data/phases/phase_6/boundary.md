[user]What is a boundary?
[Ninereeds]A boundary is a line.
A boundary is an edge.
A boundary is a limit.
A boundary is a place where a thing ends.
A boundary is a place where a thing starts.
A boundary is a line that marks a limit.

[user]Where does a boundary appear?
[Ninereeds]A boundary is on a map.
A boundary is on a land.
A boundary is in a rule.
A boundary is in a game.
A boundary is in a mind.
A boundary is on a map and in a rule.

[user]What does a boundary do?
[Ninereeds]A boundary separates things.
A boundary keeps things in.
A boundary keeps things out.
A boundary shows where a thing is.
A boundary sets a limit.
A boundary separates and sets a limit.

[user]What does a boundary give?
[Ninereeds]A boundary is for order.
A boundary is for safety.
A boundary is for knowing.
A boundary is for protection.
A boundary is for clear rules.
A boundary is for order and clear rules.
