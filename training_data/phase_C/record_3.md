[user]What does record mean?
[Ninereeds]Record is written information.
Record is preserved sound.
Record is a past achievement.
Record is a stored fact.
Record is an official account.
Record is written information and sound.

[user]Where does record appear?
[Ninereeds]Record appears in books.
Record appears in music.
Record appears in history.
Record appears in computers.
Record appears in sports.
Record is in books and music.

[user]What does record do?
[Ninereeds]Record keeps memory.
Record stores data.
Record documents events.
Record shows progress.
Record provides proof.
Record keeps memory and stores data.

[user]What does record give?
[Ninereeds]Record is for learning from past.
Record is for celebrating achievements.
Record is for proving facts.
Record is for tracking growth.
Record is for future reference.
Record is for learning and celebrating.
